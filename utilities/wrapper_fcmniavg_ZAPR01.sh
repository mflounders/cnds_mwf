...
echo "subj_code,study,fc_avg_status" > ${outputpath}/fcmaps_MNI/fcmniavg_sum_${study}_$now.csv
for i_sub in $list ;
	do
	if [ -e ${studypath}/ZAPR01_${i_sub}_AP ]; then
		${scriptpath}/fc_mni_avg.sh $i_sub $studypath $outputpath
		fc_convert="1"
	else
	...
	final_output="${i_sub},${study},${fc_convert}"
	echo $final_output >> ${outputpath}/fcmaps_MNI/fcmniavg_sum_${study}_$now.csv
done
